...
function chromiumPath() {
  // Both lookups are guarded so the throws below are reachable at all: an unset
  // LOCALAPPDATA (any non-Windows box) or a missing chromium- build reached
  // `path.join` as undefined and raised ERR_INVALID_ARG_TYPE first — and the
  // "no chrome" line threw the same way while trying to report it.
